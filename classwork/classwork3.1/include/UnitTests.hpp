...
#define ASSERT_EQ(leftValue, rightValue) \
    if ( leftValue != rightValue ) { \
        ...
        std::println("Assert failed: {} (which is equal to {}) is not equal to {} (which is equal to {}) at {}:{}", #leftValue, leftValue, #rightValue, rightValue, location.file_name(), location.line()); \
        ...
        for (auto& [name, testFunc] : tests) {
            if (testFunc) {
                ...
                try {
                    ...
                    std::println("{} {}FAILED{}: {}", name, redColor, endColor, e.what());
                }
            }
        }
